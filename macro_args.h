...
#define _ARG_CONCAT3(A, B, C) A B C
#define _ARG_BRACES(M)      _ARG_CONCAT3({, M, })
...
#define _ARG_ARRAY_INIT(MAXCNT, DEF)    _ARG_BRACES((_ARG_REPEAT(MAXCNT, DEF)))

#define _ARG_X_STRUCT_DEFAULT(TYPE, MAXCNT, NAME, SHORT, LONG, HINT, DEF, DESC) \
    .ARG_FIELD_N_NAME(NAME) = 0, \
    _ARG_IF_ELSE(_ARG__NEED(TYPE), \
                 .ARG_FIELD_NAME(NAME) = _ARG_ARRAY_INIT(MAXCNT, DEF), .ARG_FIELD_N_NAME(NAME) = 0),
